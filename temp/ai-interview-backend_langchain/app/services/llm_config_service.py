...
class LLMConfigService:
    ...
    async def test_config(data: LLMConfigTestRequest, db: AsyncSession = None) -> LLMConfigTestResponse:
        ...
        try:
            response = await test_model.ainvoke([HumanMessage(content="hi")])
            latency_ms = int((time.monotonic() - start) * 1000)
            model_returned = response.response_metadata.get("model", None) if hasattr(response, "response_metadata") else None
